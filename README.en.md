...
Rebuild the photograph as one matte iced-cookie bas-relief: round, miniature, with generous white space.
...
For turning an everyday photograph into a warm collectible miniature: personal archives, independent publishing, exhibition studies and lifestyle brands. The upper half keeps a recognisable source photograph; the lower half is not a cookie filter on the photo. It first reads meaning and relations, then rebuilds the subject and its necessary setting as one iced, fondant, soft-clay bas-relief.

The result should feel simple, complete, poetic, warm and collectible: a small stamp on abundant paper, with bright soft icing colour. Avoid literal copies, filled backgrounds, plastic highlights, cheap cartoons and template icons.
...
Restart the agent session after installation, then invoke `$xxd-panel-226`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-226 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
...
/xxd-panel-226 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-226-prompt.en.md) or [Chinese](references/xxd-panel-226-prompt.zh-CN.md) runtime adapter.
...
- `design-only`: the full canvas contains only Panel 226's designed translation; the photograph remains a non-visible reference.
